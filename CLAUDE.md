...
Tool-specific notes for Claude:

- If given a topic via a slash command, CLI arg, or chat message, treat the
  whole thing as the "Input" described in `INSTRUCTIONS.md` Section 1.
- Write the file directly to the path the user specifies, nested under
  `notes/` (see `INSTRUCTIONS.md` Section 2), using your file tools — don't
  just print the Markdown in chat unless the user explicitly asks to preview
  it first.
